...
- **Configurações** (rota `config`): seções em `configSections`. Duas seções especiais além das linhas genéricas (`cfgDefs`): **Tela de login** (`state.loginCfg` + `loginDefaults`) personaliza a tela de login AO VIVO — marca, headline, descrição, accent/painel (swatches), chips, com pré-visualização; e **Avisos & comunicados** (`state.announces` + `annTypes`: release/novidade/aniversário/manutenção/comunicado) — canal de comunicação com CRUD (composer tipo/título/mensagem/público/data/link/publicado/fixar). Avisos ativos aparecem em **banner dispensável** no topo do conteúdo (fixados primeiro) e no **feed do sino** (topbar). Expor sempre `notifOpen` no retorno do renderVals.
